...
def epsilon_greedy(state_vector, theta, epsilon):
    ...
    # TODO Your code here
    if epsilon==1:  
        action_index, object_index = index2tuple(np.random.randint(NUM_OBJECTS*NUM_ACTIONS))
    else:
        q_value = (theta @ state_vector)
        max_value = np.max(q_value)
        result = np.where(q_value == max_value)[0][0]
        action_index = result // NUM_OBJECTS
        object_index = result % NUM_OBJECTS
