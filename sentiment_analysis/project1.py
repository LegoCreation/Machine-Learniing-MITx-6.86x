...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    # Your code here
    c= 0
    q= (labels.transpose()*(np.dot(feature_matrix,theta.transpose())+theta_0)).astype(float)
    dim=np.size(q,0)
    A = np.zeros((dim,1))
    
    for i in q:
        A[c]=np.maximum(0,1-i)
        c=c+1
...
def perceptron_single_step_update(
    ...
    """
    # Your code here
    new_theta = np.zeros(shape=(np.shape(current_theta)))
    new_theta_0 = 0
...
def perceptron(feature_matrix, labels, T):
    ...
    """
    # Your code here
    current_theta = np.zeros(shape=(np.shape(feature_matrix[0])))
    current_theta_0 = 0
...
def average_perceptron(feature_matrix, labels, T):
    ...
    # Your code here
    current_theta = np.zeros(shape=(np.shape(feature_matrix[0])))
    sum_theta = np.zeros(shape=(np.shape(feature_matrix[0])))
    current_theta_0 = 0
    sum_theta_0 = 0
    ...
    total_loops = T*feature_matrix.shape[0]
    return tuple((sum_theta/total_loops, sum_theta_0/total_loops))
#pragma: coderesponse end
...
def classify(feature_matrix, theta, theta_0):
    ...
    classification =np.array(np.size(prediction,0))
    classification = 2*(prediction>0)-1
...
def classifier_accuracy(
    ...
    classification_train = classify(train_feature_matrix, theta_train, theta_0_train)
    classification_val = classify(val_feature_matrix, theta_train, theta_0_train)
    train_accuracy = accuracy(classification_train, train_labels)
    val_accuracy = accuracy(classification_val, val_labels)
