...
class NeuralNetwork():
    ...
    def train(self, x1, x2, y):
        ...
        hidden_layer_error = (activated_output - y) * np.transpose(self.hidden_to_output_weights)  # TODO (3 by 1 matrix)

        bias_gradients = (activated_output - y) * np.multiply(np.transpose(self.hidden_to_output_weights), np.vectorize(rectified_linear_unit_derivative) (hidden_layer_weighted_input)) # TODO
        input_to_hidden_weight_gradients =  np.multiply(hidden_layer_error, np.vectorize(rectified_linear_unit_derivative)(hidden_layer_weighted_input)) @ np.transpose(input_values) # TODO
        hidden_to_output_weight_gradients = output_layer_activation_derivative(output) * output_layer_error * np.transpose(hidden_layer_activation) # TODO
